"""
ConfigManager - Handles loading and validating configuration for the CustomerAI framework.
...
class ConfigManager:
    """
    Handles loading and validating configuration for the CustomerAI framework.
    """
    ...
    def __init__(self, config_path: Optional[str] = None, env_file: Optional[str] = None):
        ...
        self.config_path = config_path or os.environ.get(
            "CUSTOMERAI_CONFIG", "configs/default.yaml"
        )
        self.env_file = env_file or os.environ.get("CUSTOMERAI_ENV", ".env")
    ...
    def _apply_env_overrides(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Mode settings
        if "CUSTOMERAI_MODE_TYPE" in os.environ:
            if "mode" not in config:
                config["mode"] = {}
            config["mode"]["type"] = os.environ["CUSTOMERAI_MODE_TYPE"]
        if "CUSTOMERAI_PRODUCTION_READY" in os.environ:
            if "mode" not in config:
                config["mode"] = {}
            config["mode"]["production_ready"] = (
                os.environ["CUSTOMERAI_PRODUCTION_READY"].lower() == "true"
            )
